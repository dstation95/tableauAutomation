...
# ============================================================
# Main Function: executeTitleChange(task_name, title_name)
# ============================================================
def executeTitleChange(task_name, title_name):
    """
    Identifies the UI element associated with task_name (using JSON examples),
    double-clicks it to activate title editing, waits briefly, then simulates:
      - a Ctrl+A to select all text,
      - a Backspace to clear the current title,
      - and types in the new title (title_name).
    
    Args:
      task_name (str): The task prompt used to locate example JSON files.
      title_name (str): The new title to type in.
    """
    ...
    input_examples_file = os.path.join(clicks_dir, f"{task_name_normalized}_input.json")
    output_examples_file = os.path.join(clicks_dir, f"{task_name_normalized}_output.json")
    ...
    # (Optional) Uncomment below to save a screenshot for debugging:
    # new_screenshot = main_window.capture_as_image()
    # new_screenshot_filename = f"{task_name_normalized}_current.png"
...
{example_text}
Now, given the current UI snapshot and following the examples, return ONLY the runtime_id of the UI element that should be double-clicked to change the title.
Current UI snapshot:
{new_snapshot_str}
"""
    ...
    # ------------------------------------------------------------
    # Find the Target Element and Double-Click It
    # ------------------------------------------------------------
    ...
    if target_elem is not None:
        ...
        try:
            print("Attempting double-click on target element...")
            target_elem.double_click_input()  # Double-click the element to open title editing.
        except Exception as e:
            print("double_click_input() failed with error:", e)
    else:
        print("Could not locate the element with the predicted runtime ID.")
        sys.exit(1)
    
    # ------------------------------------------------------------
    # Wait Briefly and Simulate Title Editing Keystrokes
    # ------------------------------------------------------------
    time.sleep(0.2)  # Wait 0.2 seconds for the title field to become active.
    # print("Simulating Ctrl+A, Backspace, and typing new title...")
    pyautogui.hotkey('ctrl', 'a')  # Select all text in the title field.
    pyautogui.press('backspace')   # Clear the field.
    pyautogui.write(title_name, interval=0.05)  # Type the new title.
    # print("Title change complete.")

# ============================================================
# Main Guard: Call executeTitleChange() if run directly.
# ============================================================
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        task_name_arg = sys.argv[1]
        title_name_arg = sys.argv[2]
    else:
        task_name_arg = input("Enter the task name (e.g., 'swap_x_and_y'): ").strip()
        title_name_arg = input("Enter the new title: ").strip()
    
    executeTitleChange(task_name_arg, title_name_arg)
